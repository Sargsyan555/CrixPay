...
export default function Header() {
  ...
  return (
    <header
      className="flex z-20 items-center fixed top-0 left-0 w-full shadow-md h-[96px]"
      style={{ background: "rgba(7, 24, 44, 0.7)" }}
    >
      <div className="container mx-auto flex justify-between items-center px-4">
        ...
        <nav className="hidden md:flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-8">
          <Link
            href="#about"
            className="hover:text-gray-400 font-roboto text-[18px] text-white"
          >
            About
          </Link>
          <Link
      ...
      {isMenuOpen && (
        <div className="absolute top-[96px] left-0 w-full h-[400px] bg-[#07182CB2] bg-opacity-90 p-4 flex flex-col items-center space-y-6">
          <Link
            href="/about"
            className="block hover:text-gray-400 font-roboto text-[18px] text-white"
          >
            About
          </Link>
          <Link
